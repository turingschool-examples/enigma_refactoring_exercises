...
class DecryptTest < Minitest::Test
  ...
  def test_formats_the_key
    decrypt_runner = Decrypt.new("58128", "110315")
    result =[5, 8, 1, 2, 8]
    assert_equal result, decrypt_runner.key
  end
  ...
  def test_it_turns_key_to_rotation_pairs
    ...
    decrypt_runner.format_the_key("12345")
    result= [12, 23, 34, 45]
    assert_equal result, decrypt_runner.turn_key_to_paired_key
  end

  def test_it_can_properly_format_the_date
    decrypt_runner = Decrypt.new("12345", "110315")
    result = [9, 2, 2, 5]
    assert_equal result, decrypt_runner.formatted_date
  end
